document.addEventListener('DOMContentLoaded', () => {
    ...
    function moveTo(index) {
        // Garantir que o índice esteja dentro do intervalo dos cartões originais
        if (index >= totalOriginalCards * 2) {
            index = 0;
        } else if (index < 0) {
            index = totalOriginalCards * 2 - 1;
        }
    ...
    prevButton.addEventListener('click', () => {
        ...
        if (currentIndex === 0) {
            track.style.transition = 'none';
            moveTo(totalOriginalCards * 2 - 1);
            
            // Pequeno delay para o navegador processar o pulo antes de animar
            setTimeout(() => moveTo(totalOriginalCards * 2 - 2), 20);
        } else {
            moveTo(currentIndex - 1);
        }
    });
